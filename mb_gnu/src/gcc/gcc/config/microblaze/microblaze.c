...
/* Microblaze specific machine attributes.
   interrupt_handler - Interrupt handler attribute to add interrupt prologue and 
   		       epilogue and use appropriate interrupt return.
      ...
      if (microblaze_valid_base_register_p (xplus0, mode, strict)) {
        ...
        if (GET_CODE (xplus1) == CONST_INT) {
          ...
          return true;
		   ...
		   && microblaze_valid_index_register_p (xplus1, mode, strict) 
                             ...
                             rtx *loop_reg, rtx *loop_mem)
{
  ...
  *loop_mem = change_address (mem, BLKmode, *loop_reg);
  set_mem_align (*loop_mem, MIN ((HOST_WIDE_INT)MEM_ALIGN (mem), length * BITS_PER_UNIT));
}
  ...
  int *total)
{
  ...
  switch (code) {
	...
	if (optimize_size && INTVAL (XEXP (x, 1)) > 5) *total -= 2;
	if (!optimize_size && INTVAL (XEXP (x, 1)) > 17) *total -= 2;
      }
      else 
...
override_options (void)
{
  ...
  if (ver < 0) {
    ...
    /* We agree to use 5 pipe-stage model even on area optimized 3 pipe-stage variants. */
    microblaze_select_flags &= ~(MICROBLAZE_MASK_NO_UNSAFE_DELAY);
    microblaze_no_unsafe_delay = 0;
    ...
    if (microblaze_version_compare (microblaze_select_cpu, "v5.00.a") == 0 ||
        ...
        /* Pattern compares are to be turned on by default only when compiling for MB v5.00.'z' */
  ...
  int letter)		/* %<letter> or 0 */
{
  ...
  else if (letter == 'i')
  {
     if (code == MEM)
     {
       ...
       if (!microblaze_classify_address (&info, XEXP (op, 0), GET_MODE (op), 1))
  ...
  switch (info.type) {
    ...
    case ADDRESS_REG_INDEX:
      if (REGNO (info.regA) == 0)
        /* Make rB == r0 instead of rA == r0. This helps reduce read port congestion */
...
void
microblaze_declare_object (FILE *stream, const char *name, const char *section,
  const char *fmt, int size)
{
...
{
  rtx base_reg_rtx, reg_rtx, mem_rtx, /* msr_rtx, */ isr_reg_rtx = 0, isr_mem_rtx = 0;
...
microblaze_expand_epilogue (void)
{
  ...
  if (fsiz > 0)
  {
    /* Restore SUB_RETURN_ADDR_REGNUM at first. This is to prevent the sequence
       of load-followed by a use (in rtsd) in every prologue. Saves a load-use 
       stall cycle  :)   This is also important to handle alloca. (See comments 
       for if (frame_pointer_needed) below */
    ...
    /* It is important that this is done after we restore the return address 
       register (above).  When alloca is used, we want to restore the sub-routine 
       return address only from the current stack top and not from the frame 
       pointer (which we restore below). (frame_pointer + 0) might have
...
microblaze_expand_shift (enum shift_type dir ATTRIBUTE_UNUSED, rtx operands[])
{
  gcc_assert ((GET_CODE (operands[2]) == CONST_INT) 
  	      || (GET_CODE (operands[2]) == REG) || (GET_CODE (operands[2]) == SUBREG));
  ...
  gcc_assert ((GET_CODE (operands[0]) == REG) || (GET_CODE (operands[0]) == SUBREG)
