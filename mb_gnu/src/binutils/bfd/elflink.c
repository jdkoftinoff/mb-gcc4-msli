...
bfd_elf_discard_info (bfd *output_bfd, struct bfd_link_info *info)
{
  ...
  for (abfd = info->input_bfds; abfd != NULL; abfd = abfd->link_next)
	...
	stab = NULL;
      ...
      loc = bfd_get_section_by_name (abfd, ".debug_loc");
      if (loc != NULL
	  && (loc->size == 0
	      || bfd_is_abs_section (loc->output_section)))
	loc = NULL;
      ...
      if (eh != NULL)
	{
      ...
      if (loc != NULL)
        {
	  cookie.rels = NULL;
	  count = loc->reloc_count;
	  if (count != 0)
	    cookie.rels = _bfd_elf_link_read_relocs (abfd, loc, NULL, NULL,
						     info->keep_memory);
	  cookie.rel = cookie.rels;
	  cookie.relend = cookie.rels;
	  if (cookie.rels != NULL)
	  ...
	  if (_bfd_dwarf2_fixup_section_debug_loc (abfd, info, loc,
						   bfd_elf_reloc_symbol_deleted_p,
						   &cookie))
	    ret = TRUE;

	  if (cookie.rels != NULL
	      && elf_section_data (loc)->relocs != cookie.rels)
	    free (cookie.rels);
        }
