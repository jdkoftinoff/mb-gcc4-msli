...
/* Fixup .debug_loc section references which have been changed by relaxation.  
   .debug_loc entries which do not have relocation entries refer to .text section.

   If there is a relocation entry, it has already been updated by relaxation. 

   (It isn't clear that the data needs to be updated when there are relocation
   entries, but we do it anyway for consistency between entries which have 
   relocation entries and those which do not.) */

bfd_boolean
_bfd_dwarf2_fixup_section_debug_loc (bfd *abfd, 
				     struct bfd_link_info *info, 
				     asection *debug_loc_sec,
				     bfd_boolean (*reloc_symbol_deleted_p) (bfd_vma, void *),
    				     struct elf_reloc_cookie *cookie)
{
  bfd_byte *locbuf;
  Elf_Internal_Rela *irel = cookie->rel, *irelend = cookie->relend; 
  Elf_Internal_Shdr *symtab_hdr;
  Elf_Internal_Sym *isymbuf;
  int symcount, offset, ret = FALSE;

  if (debug_loc_sec->size == 0)
    {
      /* This file does not contain .debug_loc information.  */
      return FALSE;
    }
  ...
  if ((debug_loc_sec->output_section != NULL
       && bfd_is_abs_section (debug_loc_sec->output_section)))
    {
  ...
  /* Read .debug_loc contents. */
    if (!bfd_malloc_and_get_section (abfd, debug_loc_sec, &locbuf))
    goto error_return;

  for (offset = 0; offset < debug_loc_sec->size; )
    {
       bfd_vma begin, end, nbegin, nend;
       asection *ref_sec, *text_sec = bfd_get_section_by_name (abfd, ".text");
       unsigned long r_symndx = 0, r_sect = 0;
      ...
      // printf ("%8.8x %8.8lx %8.8lx", offset, begin + base_address, end + base_address);
	...
	{
	  while ((irel->r_offset < offset) && (irel < irelend)) irel++;
	  if (irel->r_offset != offset)
	    ref_sec = text_sec;
          else 
	    {
	      r_symndx = ELF32_R_SYM (irel->r_info);
	      if (r_sect != isymbuf[r_symndx].st_shndx) 
		{
	          r_sect = isymbuf[r_symndx].st_shndx;
                  ref_sec = bfd_section_from_elf_index (abfd, r_sect);
		}
            }
	}
      // printf (" -- %s", ref_sec->name);

      /* Fix base address entry. */
      if (begin == -1UL && end != -1UL)
        {
          base_address = end - calc_fixup (end, ref_sec);
          // printf ("%8.8x %8.8lx %8.8lx (base address)", offset, begin, end);
	  if (base_address != end)
	    {
	      // printf("\nreplaced %8.8x %8.8x", (int) begin, (int) base_address);
	      bfd_put_32 (abfd, base_address, locbuf + offset + 4);
	      BFD_ASSERT ((irel+1)->r_addend == base_address);
	      ret = TRUE;
	    }
	  continue;
        }
      ...
      /* Fix location list entry. */
      nbegin = begin - calc_fixup (begin + base_address, ref_sec) - base_address;
      nend = end - calc_fixup (end + base_address, ref_sec) - base_address;

      if (nbegin != begin || nend != end)
        {
	  // printf("\nreplaced %8.8x %8.8x", (int) nbegin, (int) nend);
	  bfd_put_32 (abfd, nbegin, locbuf + offset);
	  bfd_put_32 (abfd, nend, locbuf + offset + 4);
	  ret = TRUE;
	}

      // printf ("\n");

      /* Check that contents and reloc match. */
      if (irel && irel->r_offset == offset) 
	{
           BFD_ASSERT (irel->r_addend == nbegin);
           BFD_ASSERT ((irel+1)->r_addend == nend);
	}

      /* Skip over location expression. */
      blklen = bfd_get_16 (abfd, locbuf + offset + 8);
      offset += (4 + 4 + 2) + blklen;
    }
  // printf("\n");
